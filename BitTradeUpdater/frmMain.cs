...
namespace BitTradeUpdater
{
    public partial class frmMain : Form
    {
        ...
        private async Task<int> UpdateFirebaseRemote()
        {
            ...
            var db = FirestoreDb.Create("bittrade-0");
            DocumentReference docRef = db.Document("app/config");
            Dictionary<FieldPath, object> updates = new Dictionary<FieldPath, object>
                {
                    { new FieldPath("devVersion"), remoteVer },
                    { new FieldPath("devUrlpath"), devUrlpath },
